...
{
    public class BulkUploadItemCommand : IRequest<Result>
    {
        public IFormFile? File { get; set; }
        public CrudOperations? Operation { get; set; } = CrudOperations.Create;
    }

    public class BulkUploadItemService : IRequestHandler<BulkUploadItemCommand, Result>
    {
        ...
        public BulkUploadItemService(IAppDbContext db, ILangueageService lang, IActor actor)
        {
            _dbContext = db;
            _lang = lang;
            _actor = actor;
        ...
        public async Task<Result> Handle(BulkUploadItemCommand request, CancellationToken ct)
        {
        ...
        async Task<IXLWorksheet?> GetWorksheet(BulkUploadItemCommand request, CancellationToken ct)
        {
        ...
        ItemBulkUpload? GetBulkRecord(BulkUploadItemCommand request)
        {
            if(bulkRecord is not null)
                return bulkRecord;

            bulkRecord = _dbContext.ItemBulkUploads.FirstOrDefault(x => x.UserId == _actor.UserId && x.CompanyId == _actor.CompanyId && !x.IsDeleted && x.Operation == request.Operation);
            return bulkRecord;
        }

        async Task<Result> Validate(BulkUploadItemCommand request, CancellationToken ct)
        {
