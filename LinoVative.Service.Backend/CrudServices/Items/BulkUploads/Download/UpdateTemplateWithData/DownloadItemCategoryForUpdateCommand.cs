...
namespace LinoVative.Service.Backend.CrudServices.Items.BulkUploads.Download.UpdateTemplateWithData
{
    public class DownloadItemCategoryForUpdateCommand : IRequest<Result>
    {
        public List<FilterCondition> Filter { get; set; } = new();
    }

    public class DownloadItemCategoryForUpdateHandler : QueryServiceBase<ItemCategory, DownloadItemCategoryForUpdateCommand>, IRequestHandler<DownloadItemCategoryForUpdateCommand, Result>
    {
        private readonly IBulkOperationTemplateFactory _bulkOperationFactory;
        public DownloadItemCategoryForUpdateHandler(IBulkOperationTemplateFactory bulkOpFactory, IAppDbContext dbContext, IActor actor, IMapper mapper, IAppCache appCache) :
        ...
        public async Task<Result> Handle(DownloadItemCategoryForUpdateCommand request, CancellationToken ct)
        {
            var filter = request.Filter.FirstOrDefault(x => x.Field == "ItemGroup.Name");
            if (filter is not null) filter.Field = "Group.Name";

            var wb = await _bulkOperationFactory.GetService(BulkOperationTemplateType.Category_Update).GetTemplate();
