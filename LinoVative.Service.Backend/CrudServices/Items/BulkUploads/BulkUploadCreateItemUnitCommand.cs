...
{
    public class BulkUploadCreateItemUnitCommand : IRequest<Result>
    {
        public IFormFile? File { get; set; }
    }

    public class BulkUploadCreateItemUnitService : IRequestHandler<BulkUploadCreateItemUnitCommand, Result>
    {
        ...
        public BulkUploadCreateItemUnitService(IAppDbContext db, ILangueageService lang, IActor actor)
        {
            _dbContext = db;
            _lang = lang;
            _actor = actor;
        ...
        public async Task<Result> Handle(BulkUploadCreateItemUnitCommand request, CancellationToken ct)
        {
            ...
            if(exisitingUpload is null)
            {
                ...
                exisitingUpload = new ItemUnitBulkUpload() { headerColum1 = header1, headerColum2 = header2, CompanyId = _actor.CompanyId, UserId = _actor.UserId, Operation = CrudOperations.Create};
        ...
        async Task<IXLWorksheet?> GetWorksheet(BulkUploadCreateItemUnitCommand request, CancellationToken ct)
        {
        ...
        ItemUnitBulkUpload? GetBulkRecord(BulkUploadCreateItemUnitCommand request)
        {
            if(bulkRecord is not null)
                return bulkRecord;

            bulkRecord = _dbContext.ItemUnitBulkUploads.FirstOrDefault(x => x.UserId == _actor.UserId && x.CompanyId == _actor.CompanyId && !x.IsDeleted && x.Operation == CrudOperations.Create);
            return bulkRecord;
        }

        async Task<Result> Validate(BulkUploadCreateItemUnitCommand request, CancellationToken ct)
        {
