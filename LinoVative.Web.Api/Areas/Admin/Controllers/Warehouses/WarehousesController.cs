...
namespace LinoVative.Web.Api.Areas.Admin.Controllers.Shifts
{
    public class WarehousesController : PrivateAPIBaseController
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreateWarehouseCommand c, CancellationToken token)
        {
            try
            {
        ...
        [HttpPut]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Update([FromBody] UpdateWarehouseCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
        ...
        public async Task<IActionResult> Delete([FromRoute]Guid id, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(new DeleteWarehouseCommand() { Id = id}, token);
                return StatusCode((int)result.Status, result);
            }
            catch (Exception ex)
            {
