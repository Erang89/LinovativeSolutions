...
namespace LinoVative.Web.Api.Areas.Admin.Controllers.People
{
    public class PeopleController : PrivateAPIBaseController
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreatePersonCommand c, CancellationToken token)
        {
            try
            {
        ...
        [HttpPut]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Update([FromBody] UpdatePersonCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
        ...
        public async Task<IActionResult> Delete([FromRoute]Guid id, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(new DeletePersonCommand() { Id = id}, token);
                return StatusCode((int)result.Status, result);
            }
            catch (Exception ex)
            {
