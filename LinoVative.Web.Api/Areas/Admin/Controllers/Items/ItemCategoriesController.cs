...
using LinoVative.Service.Backend.CrudServices.Items.ItemCategries;
...
namespace LinoVative.Web.Api.Areas.Admin.Controllers.Items
{
    public class ItemCategoriesController : PrivateAPIBaseController
    {
        ...
        [Route("{id}")]
        [HttpGet]
        [ProducesResponseType(typeof(APIListResponse<ItemCategoryInputDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Get(Guid id, CancellationToken token)
        {
            try
            {
                var c = new GetItemCategoryForUpdateCommand() { Id = id };
        ...
        public async Task<IActionResult> Create([FromBody] CreateItemCategoryCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
        ...
        [HttpPut]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Update([FromBody] UpdateItemCategoryCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
        ...
        public async Task<IActionResult> Delete([FromRoute]Guid id, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(new DeleteItemCategoryCommand() { Id = id}, token);
                return StatusCode((int)result.Status, result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> DeleteAll([FromBody] BulkDeleteItemCategoryCommand cmd, CancellationToken token)
        {
            try
            {
