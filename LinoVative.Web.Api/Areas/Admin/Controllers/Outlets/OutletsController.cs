...
namespace LinoVative.Web.Api.Areas.Admin.Controllers.Outlets
{
    public class OutletsController : PrivateAPIBaseController
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreateOutletCommand c, CancellationToken token)
        {
            try
            {
        ...
        [HttpPut]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Update([FromBody] UpdateOutletCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
        ...
        public async Task<IActionResult> Delete([FromRoute]Guid id, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(new DeleteOutletCommand() { Id = id}, token);
                return StatusCode((int)result.Status, result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> DeleteAll([FromBody] BulkDeleteOutletCommand cmd, CancellationToken token)
        {
            try
            {
