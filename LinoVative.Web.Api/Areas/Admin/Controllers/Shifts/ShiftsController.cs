...
namespace LinoVative.Web.Api.Areas.Admin.Controllers.Shifts
{
    public class ShiftsController : PrivateAPIBaseController
    {
        ...
        public async Task<IActionResult> Get(Guid id, CancellationToken token)
        {
            try
            {
                var c = new GetShiftForUpdateCommand() { Id = id };
        ...
        public async Task<IActionResult> Create([FromBody] CreateShiftCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
        ...
        [HttpPut]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Update([FromBody] UpdateShiftCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
        ...
        public async Task<IActionResult> Delete([FromRoute]Guid id, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(new DeleteShiftCommand() { Id = id}, token);
                return StatusCode((int)result.Status, result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> DeleteAll([FromBody] BulkDeleteShiftCommand cmd, CancellationToken token)
        {
            try
            {
