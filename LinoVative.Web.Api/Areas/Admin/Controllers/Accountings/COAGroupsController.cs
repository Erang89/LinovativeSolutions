...
namespace LinoVative.Web.Api.Areas.Admin.Controllers.Accountings
{
    public class COAGroupsController : APIBaseController
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreateCOAGroupCommand c, CancellationToken token)
        {
            try
            {
        ...
        [HttpPut]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Update([FromBody] UpdateCOAGroupCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
