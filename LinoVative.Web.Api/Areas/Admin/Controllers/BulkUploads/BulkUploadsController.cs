...
{
    public class BulkUploadsController : PrivateAPIBaseController
    {
        public BulkUploadsController(IMediator mediator, ILogger<BulkUploadsController> logger) : base(mediator, logger)
        {
        }
        ...
        public async Task<IActionResult> UploadGroup(BulkUploadCreateItemGroupCommand c, CancellationToken token)
        {
            try
            {
        ...
        [ProducesResponseType(typeof(Result), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> UploadCategory(BulkUploadCreateItemCategoryCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
