...
namespace LinoVative.Web.Api.Areas.Admin.Controllers.Payments
{
    public class BankNotesController : PrivateAPIBaseController
    {
        ...
        [Route("{id}")]
        [HttpGet]
        [ProducesResponseType(typeof(APIListResponse<BankNoteInputDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Get(Guid id, CancellationToken token)
        {
            try
            {
                var c = new GetBankNoteForUpdateCommand() { Id = id };
        ...
        [HttpPost]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> DeleteAll([FromBody] BulkDeleteBankNoteCommand cmd, CancellationToken token)
        {
            try
            {
