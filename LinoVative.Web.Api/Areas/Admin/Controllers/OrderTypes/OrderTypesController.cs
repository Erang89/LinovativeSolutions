...
namespace LinoVative.Web.Api.Areas.Admin.Controllers.OrderTypes
{
    public class OrderTypesController : PrivateAPIBaseController
    {
        ...
        [Route("{id}")]
        [HttpGet]
        [ProducesResponseType(typeof(APIListResponse<OrderTypeInputDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Get(Guid id, CancellationToken token)
        {
            try
            {
                var c = new GetOrderTypeForUpdateCommand() { Id = id };
        ...
        public async Task<IActionResult> Create([FromBody] CreateOrderTypeCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
        ...
        [HttpPut]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Update([FromBody] UpdateOrderTypeCommand c, CancellationToken token)
        {
            try
            {
                var result = await _mediator.Send(c, token);
