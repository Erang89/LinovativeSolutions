...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class ItemsController : PrivateODataBaseController
    {
        private readonly ILogger _logger;
        public ItemsController(ILogger<ItemsController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<ItemDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllItemIQueryableCommand c, CancellationToken token)
        {
            try
            {
