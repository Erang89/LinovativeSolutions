...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class OutletAreasController : PrivateODataBaseController
    {
        private readonly ILogger _logger;
        public OutletAreasController(ILogger<OutletAreasController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<OutletAreaViewDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllOutletAreaGroupIQueryableCommand c, CancellationToken token)
        {
            try
            {
