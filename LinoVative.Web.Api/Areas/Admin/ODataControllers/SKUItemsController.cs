...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class SKUItemsController : PrivateODataBaseController
    {
        private readonly ILogger _logger;
        public SKUItemsController(ILogger<SKUItemsController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<SKUItemViewDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllSKUItemIQueryableCommand c, CancellationToken token)
        {
            try
            {
