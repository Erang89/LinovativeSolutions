...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class ItemCategoriesController : ODataControllerBaseController
    {
        private readonly ILogger _logger;
        public ItemCategoriesController(ILogger<ItemCategoriesController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<ItemCategoryViewDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllItemCategoryIQueryableCommand c, CancellationToken token)
        {
            try
            {
