...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class SuppliersController : PrivateODataBaseController
    {
        private readonly ILogger _logger;
        public SuppliersController(ILogger<SuppliersController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<SupplierDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllSupplierIQueryableCommand c, CancellationToken token)
        {
            try
            {
