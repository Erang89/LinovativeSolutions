...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class CountriesController : PublicODataBaseController
    {
        private readonly ILogger _logger;
        public CountriesController(ILogger<CountriesController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<CountryDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllCountryIQueryableCommand c, CancellationToken token)
        {
            try
            {
