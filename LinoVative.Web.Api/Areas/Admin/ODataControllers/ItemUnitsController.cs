...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class ItemUnitsController : ODataControllerBaseController
    {
        private readonly ILogger _logger;
        public ItemUnitsController(ILogger<ItemUnitsController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<ItemUnitDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllItemUnitIQueryableCommand c, CancellationToken token)
        {
            try
            {
                IQueryable<ItemUnitDto> result = await _mediator.Send(c, token);
                return Ok(result);
            }
            catch (Exception ex)
            {
