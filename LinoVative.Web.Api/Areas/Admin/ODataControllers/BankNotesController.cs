...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class BankNotesController : MainApiODataBaseController
    {
        private readonly ILogger _logger;
        public BankNotesController(ILogger<BankNotesController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<BankNoteDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllBankNoteIQueryableCommand c, CancellationToken token)
        {
            try
            {
