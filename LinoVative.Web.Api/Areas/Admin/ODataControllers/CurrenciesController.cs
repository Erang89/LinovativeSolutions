...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class CurrenciesController : PublicODataBaseController
    {
        private readonly ILogger _logger;
        public CurrenciesController(ILogger<CurrenciesController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<CurrencyDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllCurrencyIQueryableCommand c, CancellationToken token)
        {
            try
            {
