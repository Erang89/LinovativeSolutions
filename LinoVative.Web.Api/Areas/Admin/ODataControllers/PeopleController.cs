...
using LinoVative.Service.Backend.CrudServices.People;
...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class PeopleController : MainApiODataBaseController
    {
        private readonly ILogger _logger;
        public PeopleController(ILogger<PeopleController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<PersonViewDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllPersonIQueryableCommand c, CancellationToken token)
        {
            try
            {
