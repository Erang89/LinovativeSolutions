...
namespace LinoVative.Web.Api.Areas.Admin.ODataControllers
{
    ...
    public class CustomersController : PrivateODataBaseController
    {
        private readonly ILogger _logger;
        public CustomersController(ILogger<CustomersController> log) => _logger = log;



        [EnableQuery]
        [ProducesResponseType(typeof(APIListResponse<CustomerDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Post([FromBody] GetAllCustomerIQueryableCommand c, CancellationToken token)
        {
            try
            {
